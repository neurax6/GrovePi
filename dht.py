...
class Dht(object):
    ...
    def loadconfig(self,p_file):
        ...
        read = f.readline()
        self.setmu(str(read[0:len(read) - 1]))
        read = f.readline()
        self.setsleep(bool(read[0:len(read) - 1]))
        read = f.readline()
        self.seton(bool(read[0:len(read) - 1]))
        read = f.readline()
        self.setlink(read[0:len(read) - 1])
        read = f.readline()
    ...
    def readconfig(self):
        read = ""
        try:
            read = "Name:{0} Sensor numbre:{1} Measurement unit:{2} Sleep?:{3} Linked to:{4}\n Link ID:{5}".format(
